...
def normal_logpdf_tau(value, mu, tau):
    ...
    mu = np.atleast_2d(mu)
    n, k = value.shape
    constant = -0.5 * n * k * np.log(2 * np.pi) + 0.5 * n * np.log(np.linalg.det(tau))
    delta = value - mu
    return constant - 0.5 * (delta.T * np.dot(tau, delta.T)).sum()
...
class TestMatchesScipy(SeededTest):
    ...
    def test_mvnormal_indef(self):
        ...
        logp = theano.function([cov, x], logp)
        assert logp(cov_val, np.ones(2)) == -np.inf
